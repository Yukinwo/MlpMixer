...
class Mixer_struc(nn.Module):
    def __init__(self, weight, patches: int, token_dim: int, dim: int, channel_dim: int, dropout=0.):
    ...
    def forward(self, x):
        ...
        out = x * self.weight + out * (1 - self.weight)
        out2 = self.LayerNorm(out)
        out2 = self.MLP_block_chan(out2)
        out2 = out * self.weight + out2 * (1 - self.weight)
        return out2